...
<<eval=FALSE,echo=FALSE>>=
 barplot(prop.table(table(dat.HOA$HOA)),
  xlab="HOA Status",
  ylab="Relative Frequency",
  col="pink",
  main = "Count of Homes by HOA Status",
  sub = "1=Yes, 2=No")
abline(h=0)
@
\newpage
...
\newline
<<fig=TRUE, echo=FALSE>>=
dat.HOA.y<-subset(dat.HOA,dat.HOA$HOA==1) 
dat.HOA.n<-subset(dat.HOA,dat.HOA$HOA==2)
dat.HOA$HOA<-factor(dat.HOA$HOA,
                    levels=c(1,2),
                    labels=c("Yes", "No"))
ggdat<-data.frame(HOA=dat.HOA$HOA)
ggplot(data=ggdat,aes(x=HOA,fill=HOA))+
  geom_bar()+
  theme_bw()+
  geom_hline(yintercept=0)+
  xlab("HOA Status")+
  ylab("Count")+
  ggtitle("HOA Statuses")
@
\begin{table}[h]
...
\newline
<<fig=TRUE,echo=FALSE>>=
ggdat.y<-data.frame(prop.table(table(dat.HOA.y$Recycle)))
ggdat.y$HOA<-1
ggdat.n<-data.frame(prop.table(table(dat.HOA.n$Recycle)))
ggdat.n$HOA<-2
ggdat.recycle<-rbind(ggdat.y,ggdat.n)
names(ggdat.recycle)<-c("Recycle", "Freq", "HOA")
ggdat.recycle$HOA<-factor(ggdat.recycle$HOA,
                  levels = c(1,2),
                  labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.recycle,aes(x=Recycle,y=Freq, fill=HOA))+
    geom_col(position = position_dodge())+
    xlab("Recycling Status")+
    ylab("Number of Homes")+
    ggtitle("Recycling by HOA Status")+
    theme_bw()+
    geom_hline(yintercept = 0)+
  scale_x_discrete(labels = c("0" = "No Curbside Pickup", "1" = "Both Recycling and Trash", "2"="Only Trash"))
p2
...
<<fig=TRUE, echo=FALSE>>=
library("qqplotr")
ggdat<-data.frame(HOA=as.numeric(dat.HOA$HOA), Recycle=as.numeric(dat.HOA$Recycle))
ggplot(data=ggdat,aes(sample=Recycle))+
...
\newline
<<fig=TRUE,echo=FALSE>>=
#dat.HOA.y<-subset(dat.HOA,dat.HOA1$HOA==1)
#dat.HOA.n<-subset(dat.HOA,dat.HOA1$HOA==2)
ggdat.y<-data.frame(prop.table(table(dat.HOA.y$LawnCare)))
ggdat.y$HOA<-1
ggdat.n<-data.frame(prop.table(table(dat.HOA.n$LawnCare)))
ggdat.n$HOA<-2
ggdat.lawn<-rbind(ggdat.y,ggdat.n)
names(ggdat.lawn)<-c("LawnCare", "Freq", "HOA")
ggdat.lawn$HOA<-factor(ggdat.lawn$HOA,
                  levels = c(1,2),
                  labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.lawn,aes(x=LawnCare,y=Freq, fill=HOA))+
    ...
    theme_bw()+
    geom_hline(yintercept = 0)+
  scale_x_discrete(labels = c("1" = "Excellent", "2" = "Good", "3"="Poor", "4" = "None"))
p2
@
...
<<fig=TRUE, echo=FALSE>>=
library("qqplotr")
ggdat<-data.frame(HOA=as.numeric(dat.HOA$HOA), LawnCare=as.numeric(dat.HOA$LawnCare))
ggplot(data=ggdat,aes(sample=LawnCare))+
  stat_qq_band(alpha=0.25) +
  stat_qq_line()+
  stat_qq_point()+
  theme_bw()+
...
\newline
<<fig=TRUE,echo=FALSE>>=
#dat.HOA.y<-subset(dat.HOA1,dat.HOA1$HOA==1)
#dat.HOA.n<-subset(dat.HOA1,dat.HOA1$HOA==2)
ggdat.y<-data.frame(prop.table(table(dat.HOA.y$Trees)))
ggdat.y$HOA<-1
ggdat.n<-data.frame(prop.table(table(dat.HOA.n$Trees)))
ggdat.n$HOA<-2
ggdat.trees<-rbind(ggdat.y,ggdat.n)
names(ggdat.trees)<-c("Trees", "Freq", "HOA")
ggdat.trees$HOA<-factor(ggdat.trees$HOA,
                  levels = c(1,2),
                  labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.trees,aes(x=Trees,y=Freq, fill=HOA))+
    geom_col(position = "dodge")+
    xlab("Trees")+
    ylab("Relative Frequency")+
    ggtitle("Number of Trees by HOA Status")+
    theme_bw()+
    geom_hline(yintercept = 0)
p2
@
...
<<fig=TRUE, echo=FALSE>>=
library("qqplotr")
ggdat<-data.frame(HOA=as.numeric(dat.HOA$HOA), Trees=as.numeric(dat.HOA$Trees))
ggplot(data=ggdat,aes(sample=Trees))+
  stat_qq_band(alpha=0.25) +
  stat_qq_line()+
  stat_qq_point()+
  theme_bw()+
...
#dat.HOA.y<-subset(dat.HOA1,dat.HOA1$HOA==1)
#dat.HOA.n<-subset(dat.HOA1,dat.HOA1$HOA==2)
ggdat.y<-data.frame(prop.table(table(dat.HOA.y$Garden)))
ggdat.y$HOA<-1
ggdat.n<-data.frame(prop.table(table(dat.HOA.n$Garden)))
ggdat.n$HOA<-2
ggdat.garden<-rbind(ggdat.y,ggdat.n)
names(ggdat.garden)<-c("Garden", "Freq", "HOA")
ggdat.garden$HOA<-factor(ggdat.garden$HOA,
                  levels = c(1,2),
                  labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.garden,aes(x=Garden,y=Freq, fill=HOA))+
    geom_col(position = "dodge")+
    xlab("Garden Present")+
    ylab("Relative Frequency")+
...
<<fig=TRUE, echo=FALSE>>=
library("qqplotr")
ggdat<-data.frame(HOA=as.numeric(dat.HOA$HOA), Garden=as.numeric(dat.HOA$Garden))
ggplot(data=ggdat,aes(sample=Garden))+
  stat_qq_band(alpha=0.25) +
  stat_qq_line()+
  stat_qq_point()+
  theme_bw()+
...
\newline
<<fig=TRUE,echo=FALSE>>=
ggdat.r.lc<-data.frame(dat.HOA$Recycle,dat.HOA$LawnCare,dat.HOA$Neighborhood,dat.HOA$HOA)
names(ggdat.r.lc)<-c("Recycle", "LawnCare", "Neighborhood", "HOA")
#ggdat.r.lc$HOA<-factor(ggdat.r.lc$HOA,levels = c(1,2),labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.r.lc, aes(x=LawnCare, y=Recycle, 
                                #shape=Neighborhood, 
                                color=HOA)) +
  #geom_jitter()+
  #geom_smooth(method=lm,se=FALSE,fullrange=TRUE)+
  geom_tile(aes(fill=HOA),color="grey50")+
  xlab("Lawn Care Status")+
  ylab("Recycling Status")+
  ggtitle("Correlation between Lawn Care and Recycling by Neighborhood")+
  #scale_x_discrete(labels=c("1"="Excellent", "2"="Good", "3"="Poor", "4"="None"))+
  #scale_y_discrete(labels=c("0" = "No Curbside Pickup", "1" = "Trash and Recycling", "2" = "Only Trash Pickup"))+
  theme_bw()
p2
@
...
rlcy1<-addmargins(table(dat.HOA.y$Recycle,dat.HOA.y$LawnCare))
rlcy1
prop.table(rlcy)
rlcn<-table(dat.HOA.n$Recycle,dat.HOA.n$LawnCare)
rlcn1<-addmargins(table(dat.HOA.n$Recycle,dat.HOA.n$LawnCare))
rlcn1
prop.table(rlcn)
@
...
\newline
<<fig=TRUE,echo=FALSE>>=
ggdat.r.g<-data.frame(dat.HOA$Recycle,dat.HOA$Garden,dat.HOA$Neighborhood,dat.HOA$HOA)
names(ggdat.r.g)<-c("Recycle", "Garden", "Neighborhood", "HOA")
#ggdat.r.g$HOA<-factor(ggdat.r.g$HOA,levels = c(1,2),labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.r.g, aes(x=Garden, y=Recycle, 
                               #shape=Neighborhood, 
                               color=HOA)) +
  #geom_jitter()+
  #geom_smooth(method=lm,se=FALSE,fullrange=TRUE)+
  ...
  xlab("Garden Status")+
  ylab("Recycling Status")+
  ggtitle("Correlation between Garden Status and Recycling by Neighborhood")+
  #scale_x_discrete(labels=c("1"="Yes","2", "No"))+
  #scale_y_discrete(labels=c("0" = "No Curbside Pickup", "1" = "Trash and Recycling", "2" = "Only Trash Pickup"))+
  theme_bw()
p2
...
\newline{HOA Households}
\newline{(215 x 82)/1616=10.91}
\newline{(614 x 82)/1616=31.16}
\newline{(230 x 82)/1616=11.67}
\newline{(215 x 977)/1616=129.98}
\newline{(614 x 977)/1616=257.23}
\newline{(230 x 977)/1616=139.05}
\newline
\newline{Non HOA Households}
\newline{(59 X 56)/1616=2.04}
\newline{(373 X 56)/1616=12.93}
\newline{(125 X 56)/1616=4.33}
\newline{(59 X 501)/1616=18.29}
\newline{(373 X 501)/1616=115.64}
\newline{(125 X 501)/1616=38.75}
...
\newline
<<fig=TRUE,echo=FALSE>>=
ggdat.r.t<-data.frame(dat.HOA$Recycle,dat.HOA$Trees,dat.HOA$Neighborhood,dat.HOA$HOA)
names(ggdat.r.t)<-c("Recycle", "Trees", "Neighborhood", "HOA")
#ggdat.r.t$HOA<-factor(ggdat.r.t$HOA,levels = c(1,2),labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.r.t, aes(x=Recycle, y=Trees, 
                               #shape = Neighborhood,
                               fill=HOA)) +
  #geom_bar(stat="identity", position="dodge")+
  #geom_smooth(method=lm,se=FALSE,fullrange=TRUE)+
  geom_tile(aes(fill=HOA),color="grey50")+
  ylab("Number of Trees in Yard")+
  xlab("Recycling Status")+
  ggtitle("Correlation between Number of Trees and Recycling by Neighborhood")+
  #scale_x_discrete(labels=c("0" = "No Curbside Pickup", "1" = "Trash and Recycling", "2" = "Only Trash Pickup"))+
  theme_bw()
p2
@
...
\newline
<<fig=TRUE,echo=FALSE>>=
ggdat.lc.g<-data.frame(dat.HOA$LawnCare,dat.HOA$Garden,dat.HOA$Neighborhood,dat.HOA$HOA)
names(ggdat.lc.g)<-c("LawnCare", "Garden", "Neighborhood", "HOA")
#ggdat.lc.g$HOA<-factor(ggdat.lc.g$HOA,levels = c(1,2),labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.lc.g, aes(x=LawnCare, y=Garden, 
                                #shape=Neighborhood, 
                                fill=HOA)) +
  #geom_jitter()+
  #geom_smooth(method=lm,se=FALSE,fullrange=TRUE)+
  ...
  xlab("Lawn Care Status")+
  ylab("Garden Status")+
  ggtitle("Correlation between Lawn Care and Garden Status by Neighborhood")+
  #scale_x_discrete(labels=c("1"="Excellent", "2"="Good", "3"="Poor", "4"="None"))+
  #scale_y_discrete(labels=c("1"="Yes","2", "No"))+
  theme_bw()
p2
@
...
We need to check our expected counts, or the number of observations that we would expect to see in a cell if the observations were truly independent (if the null hypothesis is true). We need to ensure 80\% of expected counts are greater than 5. This is calculated by multiplying the total of row i by the total of column j and dividing this quantity by n. We can do this for HOA and non HOA households. 
\newline{HOA Households}
\newline{(377 x 82)/1616=19.13}
\newline{(250 x 82)/1616=12.68}
\newline{(194 x 82)/1616=9.84}
\newline{(238 x 82)/1616=12.08}
\newline{(377 x 977)/1616=227.93}
\newline{(250 x 977)/1616=151.14}
\newline{(194 x 977)/1616=117.29}
\newline{(238 x 977)/1616=143.89}
\newline
\newline{Non HOA Households}
\newline{(110 X 56)/1616=3.81}
\newline{(200 X 56)/1616=6.93}
\newline{(181 X 56)/1616=6.27}
\newline{(66 X 56)/1616=2.29}
\newline{(110 X 501)/1616=34.10}
\newline{(200 X 501)/1616=62.00}
\newline{(181 X 501)/1616=56.11}
\newline{(66 X 501)/1616=20.46}
...
\newline
<<fig=TRUE,echo=FALSE>>=
ggdat.lc.t<-data.frame(dat.HOA$LawnCare,dat.HOA$Trees,dat.HOA$Neighborhood,dat.HOA$HOA)
names(ggdat.lc.t)<-c("LawnCare", "Trees", "Neighborhood", "HOA")
#ggdat.lc.t$HOA<-factor(ggdat.lc.t$HOA,levels = c(1,2),labels = c("HOA", "No HOA"))

p2<-ggplot(data=ggdat.lc.t, aes(x=LawnCare, y=Trees, 
                                #shape=Neighborhood, 
                                fill=HOA)) +
  geom_bar(stat="identity", position="dodge")+
  ...
  ylab("Number of Trees in Yard")+
  xlab("Lawn Care Status")+
  ggtitle("Correlation between Number of Trees and Lawn Care Status by Neighborhood")+
  #scale_x_discrete(labels=c("1"="Excellent", "2"="Good", "3"="Poor", "4"="None"))+
  theme_bw()
p2
@
...
\newline
<<fig=TRUE,echo=FALSE>>=
ggdat.g.t<-data.frame(dat.HOA$Garden,dat.HOA$Trees,dat.HOA$Neighborhood,dat.HOA$HOA)
names(ggdat.g.t)<-c("Garden", "Trees", "Neighborhood", "HOA")
#ggdat.g.t$HOA<-factor(ggdat.g.t$HOA,levels = c(1,2),labels = c("HOA", "No HOA"))
#ggdat.g.t$Garden<-factor(ggdat.g.t$Garden,levels = c(1,2),labels = c("Yes", "No"))

p2<-ggplot(data=ggdat.g.t, aes(x=Garden, y=Trees, 
                               #shape=Neighborhood, 
                               fill=HOA)) +
  #geom_bar(stat="identity", position="dodge")+
  geom_violin()+
  ...
  ylab("Number of Trees in Yard")+
  xlab("Garden Status")+
  ggtitle("Correlation between Number of Trees and Garden Status by Neighborhood")+
  #scale_x_discrete(labels=c("1"="Yes","2", "No"))+
  theme_bw()
p2
@
