...
ggplot(data=ggdat,aes(sample=Trees))+
  ...
  ggtitle("Normality Test for Tree Data")
  facet_grid(. ~ HOA)
@
...
\newpage
\noindent\rule{16cm}{0.4pt}
\textsc{\textbf{Sustainability at the Neighborhood Level}}
Now that we have examined HOA status and sustainability factors at the 
